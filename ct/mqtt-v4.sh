...
export PCT_OPTIONS="
  ...
  $PW
"
bash -c "$(wget -qLO - https://raw.githubusercontent.com/xXDjMikeXx/Proxmox/main/ct/create_lxc.sh)" || exit
msg_info "Starting LXC Container"
pct start $CTID
msg_ok "Started LXC Container"
lxc-attach -n $CTID -- bash -c "$(wget -qLO - https://raw.githubusercontent.com/xXDjMikeXx/Proxmox/main/setup/$var_install.sh)" || exit
IP=$(pct exec $CTID ip a s dev eth0 | sed -n '/inet / s/\// /p' | awk '{print $2}')
pct set $CTID -description "# ${APP} LXC
### https://xXDjMikeXx.github.io/Proxmox/
